...
// Mendapatkan properti dan mendefinisikan emit
  ...
  <div class="col-sm-12 col-sm-6 col-xl-4">
    <div class="bg-secondary rounded p-4 overflow-auto">
      <div class="d-flex text-center align-items-center justify-content-between mb-4">
        <!-- Tanggal dan status favorit -->
        <h6 class="mb-0 text-info">Note : {{ formatDate(note.CreatedAt) }}</h6>
        <button @click="favoriteNote" class="btn">
          <i :class="note.favorite ? 'fa fa-star text-warning' : 'far fa-star'"></i>
        </button>
      </div>
      ...
      <div class="d-flex gap-2 align-items-center mt-4">
        <button @click="archiveNote" class="btn btn-small btn-success">
          {{ note.archived ? 'Unarchived' : 'Archive' }}
        </button>
        <router-link :to="{ name: 'edit-note', params: { id: note.id } }" class="btn btn-small btn-warning">
          Edit
        </router-link>
